 ...
 * 本类实现的是基本数据类型上的算法运算
 */
public class SharpDES {
	// 密钥
	private final String mWorkingkey;
	// 为了字节数组与字符串互换而使用的
	...
	// 构造函数，初始化密钥
	public SharpDES(String key) {
		this.mWorkingkey = key;
	}
	...
	private byte[] UnitDes(byte[] des_key, byte[] des_data, int flag) {
		...
		if (des_key.length != 8 || des_data.length != 8 || flag != 1
				&& flag != 0) {
		...
		// 密钥初试化成二维数组
		final int[][] KeyArray = new int[16][48];
		// 将密钥字节数组转换成二进制字节数组
	...
	private byte[] Encrypt(int[] timeData, int flag, int[][] keyarray) {
		int i;
		final byte[] encrypt = new byte[8];
		final int flags = flag;
		final int[] M = new int[64];
		final int[] MIP_1 = new int[64];
		// 特别注意：xxx[IP[i]-1]等类似变换
		for (i = 0; i < 64; i++) {
	...
	private void LeftBitMove(int[] k, int offset) {
		int i;
		// 循环移位操作函数
		final int[] c0 = new int[28];
		final int[] d0 = new int[28];
		final int[] c1 = new int[28];
		final int[] d1 = new int[28];
		for (i = 0; i < 28; i++) {
			c0[i] = k[i];
			d0[i] = k[i + 28];
		}
	...
	private void LoopF(int[] M, int times, int flag, int[][] keyarray) {
		...
		final int[] R0 = new int[32];
		final int[] L1 = new int[32];
		final int[] R1 = new int[32];
		final int[] RE = new int[48];
		final int[][] S = new int[8][6];
		final int[] sBoxData = new int[8];
		final int[] sValue = new int[32];
		final int[] RP = new int[32];
		for (i = 0; i < 32; i++) {
			L0[i] = M[i]; // 明文左侧的初始化
	...
	private byte[] ByteDataFormat(byte[] data) {
		...
		final int padlen = 8 - len % 8;// 要格式化的字节数组的长度与8的倍数的差值
		final int newlen = len + padlen;
	...
	private byte[] DesEncrypt(byte[] des_key, byte[] des_data, int flag) {
		final byte[] format_key = ByteDataFormat(des_key);// 补齐密钥字节数组的长度为8的倍数，不足元素用0补
		final byte[] format_data = ByteDataFormat(des_data);// 补齐原始数据字节数组的长度为8的倍数，不足元素用0补
		final int datalen = format_data.length;// 补齐后的原始数据字节数组的长度
		final int unitcount = datalen / 8;// 补齐后的原始数据字节数组长度是8的多少倍
		final byte[] result_data = new byte[datalen];// 用于盛放加密后的结果
		// 每一次循环，都操作8个字节（加密解密）
		for (int i = 0; i < unitcount; i++) {
			final byte[] tmpkey = new byte[8];// 真正起作用的密钥字节数组，只有8个字节
			final byte[] tmpdata = new byte[8];// 用于参与操作的数据字节数组，只有8个字节
			arraycopy(format_key, 0, tmpkey, 0, 8);
			arraycopy(format_data, i * 8, tmpdata, 0, 8);
			final byte[] tmpresult = UnitDes(tmpkey, tmpdata, flag);// 执行操作
	...
	public String encrypt(String data) {
		...
		final String sbDate = dataLength + data;// 保证原始数据的前4位为该数据的长度
		final byte[] bytekey = mWorkingkey.getBytes();// 密钥字节数组
		final byte[] bytedata = sbDate.getBytes();// 原始数据字节数组
	...
	public String decrypt(String encryptData) {
		try {
			final byte[] encryptByteArray = fromBase64(encryptData);
			final byte[] bytekey = mWorkingkey.getBytes();
			byte[] result = new byte[encryptByteArray.length];
			result = DesEncrypt(bytekey, encryptByteArray, 0);
			final String deResult = new String(result);
			final int dataLength = Integer.parseInt(deResult.substring(0, 4));
	...
	private byte[] fromBase64(String str) throws Exception {
		...
		final byte[] result = new byte[len - j + 1];
