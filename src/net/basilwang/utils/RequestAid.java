...
public class RequestAid {
	private static String TAG = "RequestAid";
	private static SaParameters mRequestHeader = new SaParameters();

	public static final String BOUNDARY = "7cd4a6d158c1";
	...
	private enum MobileFileType {
		common, image, audio
	}

	// 设置http头,如果authParam不为空，则表示当前有token认证信息需要加入到头中
	public static void setHeader(String httpMethod, HttpUriRequest request,
			SaParameters authParam, String url) throws Exception {
		SaLog.log("before setHeader", "mRequestHeader.keys="
				+ (mRequestHeader == null ? null : mRequestHeader.getKeys()));
		if (!isBundleEmpty(mRequestHeader)) {
			for (int loc = 0; loc < mRequestHeader.size(); loc++) {
				String key = mRequestHeader.getKey(loc);
				SaLog.log("setHeader", "setHeader:" + key + "-"
						+ mRequestHeader.getValue(key));
				request.setHeader(key, mRequestHeader.getValue(key));
			}
		}
		String string = System.getProperties().getProperty("http.agent")
				+ " SaAndroidSDK";
		request.setHeader("User-Agent", string);
	}

	public static boolean isBundleEmpty(SaParameters bundle) {
	...
	public static void setRequestHeader(String key, String value) {
		SaLog.log("changeheader", "key=" + key + ", value" + value);
		mRequestHeader.add(key, value);
	}

	public static void setRequestHeader(SaParameters params) {
		mRequestHeader.addAll(params);
		SaLog.log("changeheader", mRequestHeader.toStr());
		for (String key : params.getKeys()) {
			SaLog.log("changeheader",
					"key=" + key + ", value" + params.getValue(key));
		}
	}
	...
	public static String encodeUrl(SaParameters parameters) {
		if (parameters == null) {
			return "";
		}
	...
	public static String openUrl(Context context, String url, String method,
			SaParameters params) throws Exception {
		String rlt = "";
		String file = "";
		// String acName = context.getClass().getName();
		// if (acName != "SplashActivity" && acName != "LoginActivity"
		// && acName != "RegisterActivity") {
		// Log.i("coolie", "非登录界面");
		// SharpParameter.setCookie(context);
		// }
		MobileFileType type = MobileFileType.common;
		// for (int loc = 0; loc < params.size(); loc++) {
		// String key = params.getKey(loc);
		// if (key.equals("imageFile")) {
		// file = params.getValue(key);
		// params.remove(key);
		// type = MobileFileType.image;
		// }
		// if (key.equals("audio")) {
		// file = params.getValue(key);
		// params.remove(key);
		// type = MobileFileType.audio;
		// }
		// }
		SaLog.log("httprequest", url + ", " + method + ", " + params.toStr()
				+ ", " + file + ", " + type);
		if (TextUtils.isEmpty(file)) {
			rlt = openUrl(context, url, method, params, null, type);
		} else {
			rlt = openUrl(context, url, method, params, file, type);

		}
		SaLog.log("httprequest result", rlt);
		return rlt;
	}

	public static String openUrl(Context context, String url, String method,
			SaParameters params, String file, MobileFileType type)
			throws Exception {
		String result = "";
		try {
			...
			if (method.equals("GET")) {
				...
				if (!TextUtils.isEmpty(file)) {
					RequestAid.paramToUpload(bos, params);
					post.setHeader("Content-Type", MULTIPART_FORM_DATA
							+ "; boundary=" + BOUNDARY);

					if (type == MobileFileType.audio) {
						RequestAid.audioContentToUpload(bos, file);
					}
					if (type == MobileFileType.image) {
						Bitmap bf = BitmapFactory.decodeFile(file);
						RequestAid.imageContentToUpload(bos, bf);
			...
			SaLog.log(TAG, "cookie: length="
					+ request.getHeaders("Cookie").length);
			for (Header h : request.getHeaders("Cookie")) {
				SaLog.log(TAG, "cookie:" + h.getName() + "-" + h.getValue());
			}
	...
	public static InputStream downloadFile(Context context, String url,
			String method, SaParameters params) throws Exception {
		try {
			HttpClient client = getNewHttpClient(context);
			HttpUriRequest request = null;
			if (method.equals("GET")) {
			...
			setHeader(method, request, params, url);
			SaLog.log("cookie:", request.getHeaders("Cookie").toString());
			HttpResponse response = client.execute(request);
			StatusLine status = response.getStatusLine();
			int statusCode = status.getStatusCode();
	...
	public static HttpClient getNewHttpClient(Context context) {
		try {
			...
			HttpConnectionParams.setConnectionTimeout(params,
					RequestAid.SET_CONNECTION_TIMEOUT);
			HttpConnectionParams.setSoTimeout(params,
					RequestAid.SET_SOCKET_TIMEOUT);
			...
			if (!wifiManager.isWifiEnabled()) {
				...
				if (mCursor != null && mCursor.moveToFirst()) {
					...
					SaLog.log(TAG, "======9999999999999999999999====="
							+ proxyStr);
	...
	private static void audioContentToUpload(OutputStream out, String file)
			throws Exception {
		StringBuilder temp = new StringBuilder();
		temp.append(MP_BOUNDARY).append("\r\n");
		temp.append(
				"Content-Disposition: form-data; name=\"audioFile\"; filename=\"")
				.append(file).append("\"\r\n");
	 ...
	 * @return void
	 */
	private static void paramToUpload(OutputStream baos, SaParameters params)
			throws Exception {
		String key = "";
		for (int loc = 0; loc < params.size(); loc++) {
	...
	public static String encodeParameters(SaParameters httpParams) {
		if (null == httpParams || RequestAid.isBundleEmpty(httpParams)) {
			return "";
		}
